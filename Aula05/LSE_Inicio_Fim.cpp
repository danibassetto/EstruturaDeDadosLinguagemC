...
void Resta_Um(NO **Inicio, NO **Fim, int N)
{
    ...
    while (*Inicio != *Fim) // enquanto não há apenas um nó
    {
        for (int i = 0; i < N - 1; i++) // looping para "procurar" o N
        {
            q = p; // armazena o nó anterior
            p = p->prox;
        ...
        if (p == *Fim)
        {
            Rem_Fim(Inicio, Fim); // se for o fim, reaproveitamos a função Rem_Fim
            p = *Inicio;
        }
        else if (p == *Inicio)
        {
            Rem_Inicio(Inicio, Fim); // se for o inicio, reaproveitamos a função Rem_Inicio
            p = *Inicio;
        }
        else // senão.. se for no meio
        {
            q->prox = p->prox; 
            free(p);
            p = q->prox;
        }
        Imprime(*Inicio); // Imprime cada remoção
    }
}
